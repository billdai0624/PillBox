...
public class SmackCcsClient {
	...
	public String generateInviteNum(){
		String temp = "";
		for(int i = 0 ; i < 4 ; i ++){
			int count = (int)(Math.random()*26) + 65;
			char tmpchar =  (char)count;
			temp = temp + tmpchar ;
	...
	public void handleIncomingDataMessage(Map<String, Object> jsonObject) {
		...
		String phoneNum = payload.get("Phone Number");
		String userName = payload.get("User Name");
		
		String action = payload.get("Action");
		if(action.equals("Upload User Msg")){
			
			if(dbConnect.getUserName(from).isEmpty()){
				//RegIdManager.saveReg(from);
				dbConnect.storeGCM(from, phoneNum , userName);
			}
			
			
		}else if(action.equals("C2C")){
			String path = SAVE_DIR + payload.get("File Name");
			String phoneNumber = payload.get("Phone Number");
			dbConnect.updateStorage(from , phoneNumber);
			String regId = dbConnect.getRegId(phoneNumber);
			payload.remove("Phone Number");
			String msg;
			if(!regId.isEmpty()){
				payload.put("User Name", dbConnect.getUserName(from));
				payload.put(MESSAGE_KEY, "You got a message :" + payload.get("Message"));
				msg = createJsonMessage(regId, getRandomMessageId() , payload , "C2C" , null , true);
			}else{
				payload.put(MESSAGE_KEY, "invite num error ");
				msg = createJsonMessage(from, getRandomMessageId() , payload , "C2C" , null , true);
			}
